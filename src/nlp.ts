...
async function runPrompt(
    ...
    if (searchDepth > MAX_DEPTH) {
        ...
        await interaction.editReply('Sorry, the AI got stuck in a loop searching for members. Please try again and if the issue persists, contact the bot developer.').catch(errorWithId);
        return;
    }

    logWithId('Running AI prompt', messages);

    async function runPromptWithModel(model: Model): Promise<boolean> {
        logWithId(`Attempting model: ${model.name}`);
        ...
        try {
            ...
            if (error?.lastError?.statusCode === 429) {
                warnWithId(`Rate limit hit on ${model.name}`);

                if (model.rateLimitExpiry) {
                    warnWithId(`Disabling ${model.name} (and higher) until midnight PT.`);
                    // Use server time if available
                    const responseDate = error.lastError.responseHeaders?.date;
                    model.rateLimitExpiry = getNextMidnightPT(
                        responseDate ? new Date(responseDate) : new Date()
                    ).getTime();
                } else {
                    warnWithId(`Disabling ${model.name} (and higher) until next minute.`);
                    model.rateLimitExpiry = Math.ceil(Date.now() / 60000) * 60000;
                }
            } else {
                errorWithId(`Exception with model ${model.name}`, error);
            }
            return false;
        }
        model.rateLimitExpiry = undefined;
        ...
        async function replyWithModel(options: string | InteractionEditReplyOptions): Promise<void | Message> {
            ...
            if (typeof options === 'string') {
                ...
                options.content += `-# Model used: ${model.name}`;
    ...
    if (userSpecifiedModel) {
        if (await runPromptWithModel(userSpecifiedModel)) return;
        await deferReplyPromise;
        await interaction.editReply('Sorry, there was an error with the selected AI model. Please try again later.').catch(errorWithId);
        return;
    }
